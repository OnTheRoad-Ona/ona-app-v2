...
const URGENCY_CHIPS: {
  ...
  {
    id: "emergency",
    label: TOW_FINAL_COPY.emergency,
    fee: "1.25x · base + call-out",
  },
  {
    id: "remote",
    label: TOW_FINAL_COPY.remote,
...
export function TowHelpFlow({
  ...
  const goBack = () => {
    ...
    if (towScreen(leaving)?.kind === "text") {
      ...
      setDraft(
        answers[prev] && towScreen(prev)?.kind === "text" ? answers[prev] : "",
      );
    }
  };
                ...
                <div className="mb-1 mt-2 flex items-center gap-1 px-0.5">
                  ...
                  finalStep !== "location" ? (
                    <button
                      ...
                      onClick={nextFinal}
                      disabled={
                        finalStep === "photos" && photos.length < TOW_MIN_PHOTOS
                      }
                      aria-label="Next"
