...
function effectiveOf(p: Payment): string {
  ...
  if (paySt === "suspended_admin" || p.payout_suspended === true)
    return "suspended";
...
export default function PaymentManagementPage() {
  ...
  useEffect(() => {
    if (authed) {
      void loadPayments();
      const t = setInterval(() => void loadPayments(), 120_000);
  ...
  const loadDetail = useCallback(async (id: string) => {
    ...
    setError(null);
    const res = await apiCall<Detail>(
  ...
  async function withPassword(action: () => Promise<void>) {
    ...
    setMsg(null);
    setError(null);
    const unlock = await apiCall<{ expiresAt?: string }>(
      "/api/admin/care/unlock",
      { method: "POST", body: JSON.stringify({ password: password.trim() }) },
    );
    if (!unlock.ok) {
      setError(unlock.message || "Invalid password");
      setBusy(false);
      setPassword("");
      return;
    }
    ...
    badge: (type: string) => ({
      ...
      background:
        type === "released"
          ...
          : type === "held"
            ? "#fed7aa"
            : type === "pending_settlement" || type === "pending"
              ? "#fde68a"
              : type === "failed"
                ? "#fecaca"
                : type === "refunded"
    ...
    <div style={styles.container}>
      ...
      >
        <div>
          ...
          <p style={styles.sub}>
            Full control: payments, force payout, refund, cancel {adminName} (
            {adminRole})
          </p>
        </div>
      ...
      <div style={styles.grid}>
        ...
        <div style={styles.card("held")}>
          ...
          <div style={styles.cardVal}>
            {ops ? naira(ops.ona.escrowHeldMinor) : ""}
          </div>
        ...
        <div style={styles.card("pending")}>
          ...
          <div style={styles.cardVal}>
            {ops ? naira(ops.ona.pendingSettlementMinor) : ""}
          </div>
          <div style={styles.cardSub}>
            {ops ? `${ops.ona.pendingSettlementCount} jobs` : ""}
          </div>
        </div>
        <div style={styles.card("released")}>
          <div style={styles.cardLabel}>Paid to pros</div>
          <div style={styles.cardVal}>
            {ops ? String(ops.ona.releasedCount) : ""}
          </div>
        ...
        <div style={styles.card("failed")}>
          ...
          <div style={styles.cardVal}>
            {ops ? String(ops.ona.failedCount) : ""}
          </div>
        ...
        <div style={styles.card("refunded")}>
          ...
          <div style={styles.cardVal}>
            {ops ? String(ops.ona.refundedCount) : ""}
          </div>
      ...
      {opsError ? (
        <p style={{ ...styles.error, marginBottom: 16 }}>
          Balances unavailable: {opsError}
        </p>
      ) : null}
      ...
      <div style={{ display: "flex", gap: selectedId ? 16 : 0 }}>
        <div
          style={{
            flex: selectedId ? "0 0 50%" : 1,
            minWidth: selectedId ? 400 : 0,
          }}
        >
          <div style={styles.panel}>
            ...
            {loading && payments.length === 0 ? (
              <p style={styles.muted}>Loading payments...</p>
            ) : filtered.length === 0 ? (
                    ...
                    {filtered.map((p) => {
                      ...
                      return (
                        <tr
                          ...
                          style={{
                            background:
                              selectedId === p.id ? "#eff6ff" : undefined,
                            cursor: "pointer",
                          }}
                          onClick={() =>
                        ...
                        >
                          <td style={styles.td}>
                            <strong>{naira(p.amount_kobo)}</strong>
                          </td>
                          <td style={styles.td}>
                            <span style={styles.badge(e)}>
                              {p.display_status || e}
                            </span>
                          </td>
                          <td style={{ ...styles.td, ...styles.muted }}>
                            {p.provider_ref
                              ? String(p.provider_ref).slice(0, 16)
                              : ""}
                          </td>
                          <td
                            style={{
        ...
        {selectedId && (
          <div style={{ flex: "0 0 50%", minWidth: 400 }}>
            <div style={styles.panel}>
              ...
              {detailLoading ? (
                <p style={styles.muted}>Loading detail...</p>
              ) : !detail ? (
                <p style={styles.muted}>Select a payment</p>
              ) : (
                <>
                  <div
                    style={{
                  ...
                  >
                    <div>
                      ...
                      <p style={{ margin: 0, fontWeight: 700 }}>
                        {detail.parties?.customer?.name || ""}
                      </p>
                      <p style={styles.muted}>
                        {detail.parties?.customer?.phone || ""}
                        {detail.parties?.customer?.email
                          ? ` · ${detail.parties.customer.email}`
                          : ""}
                      </p>
                      <p style={{ fontSize: 12, marginTop: 6 }}>
                        Paid{" "}
                        <strong>
                          {naira(Number(detail.payment.amount_kobo) || 0)}
                        </strong>
                      </p>
                    </div>
                    <div>
                      ...
                      <p style={{ margin: 0, fontWeight: 700 }}>
                        {detail.parties?.pro?.name || ""}
                      </p>
                      <p style={styles.muted}>
                        {detail.parties?.pro?.bank?.bankName || ""} ·{" "}
                        {detail.parties?.pro?.bank?.accountName || ""} · ****
                        {detail.parties?.pro?.bank?.accountLast4 || "????"}
                      </p>
                      <p style={{ fontSize: 12, marginTop: 6 }}>
                        Pro share{" "}
                        <strong>
                          {detail.payment.pro_payout_kobo != null
                            ? naira(Number(detail.payment.pro_payout_kobo))
                            : ""}
                        </strong>
                      </p>
                    </div>
                  ...
                  >
                    ...
                    <span>
                      <span style={styles.badge(esc)}>{esc}</span> ·{" "}
                      {String(detail.payment.status)}
                    </span>
                    <span style={styles.muted}>Payment ID</span>
                    <span style={{ wordBreak: "break-all" }}>
                      {String(detail.payment.id)}
                    </span>
                    <span style={styles.muted}>Job / request</span>
                    <span style={{ wordBreak: "break-all" }}>
                      {String(detail.payment.request_id || "")}
                    </span>
                    <span style={styles.muted}>FLW transfer</span>
                    <span>
                      {detail.flwLookup?.found
                        ? `Found · ${detail.flwLookup.status || ""}`
                        : detail.flwTransfers.filter((t) =>
                              /success/i.test(String(t.status || "")),
                            ).length
                          ? "Found on FLW"
                          : "No successful transfer"}
                    </span>
                    <span style={styles.muted}>Last error</span>
                    <span style={{ color: "#b91c1c" }}>
                      {String(meta.lastReleaseError || "")}
                    </span>
                    <span style={styles.muted}>Retries</span>
                    <span>
                          ...
                          void runAction(
                            {
                              ...
                              reason: r,
                              jobId: detail.payment.request_id || undefined,
                              status: "refunded",
                            },
                    ...
                    {canForce ? (
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (
                            !window.confirm(
                              "Force payout? Same ref no double pay.",
                            )
                          )
                            return;
                          void runAction(
                            {
                              ...
                              jobId: detail.payment.request_id || selectedId,
                              reason: r,
                            },
                  ...
                  <div style={{ marginTop: 16 }}>
                    ...
                    {!detail.ledger?.length ? (
                      <p style={styles.muted}>No ledger rows.</p>
                    ) : (
                  ...
                  <div style={{ marginTop: 12 }}>
                    ...
                    {!detail.flwTransfers?.length ? (
                      <p style={styles.muted}>No FLW transfers matched.</p>
                    ) : (
                      <table style={{ ...styles.table, fontSize: 12 }}>
                        <thead>
                          <tr>
