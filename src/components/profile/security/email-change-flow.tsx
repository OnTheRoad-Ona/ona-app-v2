...
export function EmailChangeFlow({
  ...
  if (!open) {
    return (
      <SecurityField
        ...
        label="Email"
        value={currentEmail}
        badge={isEmailVerified ? "verified" : undefined}
        onChangeClick={() => {
          setOpen(true);
  ...
  const verifyIdentity = async () => {
    ...
    try {
      ...
      setStep("enter_new");
      setMsg("Identity verified. Enter your new email address.");
      setPassword("");
      setEmailCode("");
      setGName("");
    } catch {
  ...
  const doChange = async (email: string) => {
    setBusy(true);
    setErr(null);
    try {
      const res = await (
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0 flex-1">
          <p className={cn("text-[11px] font-semibold", t.muted)}>Email</p>
          <p className={cn("truncate text-[13px] font-semibold", t.ink)}>
            {step === "done" ? newEmail : currentEmail}
          </p>
        </div>
        {step === "done" ? (
          <button
