...
const URGENCY_CHIPS: {
  ...
  { id: "emergency", label: GEN_FINAL_COPY.emergency, fee: "1.25x" },
  { id: "remote", label: GEN_FINAL_COPY.remote, fee: "1.35x" },
  { id: "night", label: GEN_FINAL_COPY.night, fee: "1.5x" },
];

type FinalStep = "urgency" | "photos" | "voice" | "location" | "material";

const FLOW_SESSION_KEY = "ona-generator-flow-session";
/** Guard against exceeding the ~5 MB sessionStorage quota with media data URLs. */
const FLOW_SESSION_MAX_BYTES = 3_000_000;

interface GeneratorFlowSnapshot {
  stack: string[];
...
export function GeneratorHelpFlow({
  isLight,
  onExit,
}: {
  isLight: boolean;
  ...
  const [route, setRoute] = useState<GenRoute | null>(null);
  const [chosenTrade, setChosenTrade] = useState<ProService>("generator");
  ...
  useEffect(() => {
    if (!hydrated) return;
    const snap: GeneratorFlowSnapshot = {
      stack,
      answers,
      draft,
      urgency,
  ...
  const push = (next: string, nextAnswers: Record<string, string>) => {
    ...
    if (next === "confirm" || next === "final") {
      const resolved = resolveGeneratorRoute(nextAnswers);
      setRoute(resolved);
  ...
  const pick = (optionId: string, label: string) => {
    ...
    push(nextGeneratorScreen(step, optionId, nextAnswers), nextAnswers);
  };
  ...
  const submitText = () => {
    ...
    setAnswers(nextAnswers);
    setDraft("");
    push(nextGeneratorScreen(step, draft.trim(), nextAnswers), nextAnswers);
  };
  ...
  const goBack = () => {
    ...
    if (genScreen(leaving)?.kind === "text") {
  ...
  const onPhotos = async (files: FileList | null) => {
    ...
    const room = GEN_MAX_PHOTOS - photos.length;
    if (room <= 0) {
    ...
    try {
      ...
      setPhotos((prev) => [...prev, ...next].slice(0, GEN_MAX_PHOTOS));
      clearAdvanceTimer();
      if (total >= GEN_MIN_PHOTOS) {
  ...
  const send = async () => {
    ...
    const problem = [
      ...
      composeGeneratorProblem(answers, extra, landmark),
    ]
      .filter(Boolean)
      .join("\n");
    const payload = {
  ...
  const photosIncomplete = photos.length < GEN_MIN_PHOTOS;
  const generatorSendDisabled =
    finalStep === "material" &&
    (supplyChoice === null || !canAdvanceText(size));
          ...
          <div className="flex min-h-0 flex-1 flex-col">
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain scrollbar-hide">
              <p className={cn("mt-2 px-0.5 pb-2 text-[14px] font-bold capitalize leading-snug", ink)}>
                {finalStep === "urgency"
                  ...
                  : finalStep === "photos"
                    ? GEN_FINAL_COPY.photos
                    : finalStep === "voice"
                      ? GEN_FINAL_COPY.voice
                      : finalStep === "location"
                        ? GEN_FINAL_COPY.location
                        : GEN_FINAL_COPY.size}
              </p>
              <div className={cn("rounded-[4px] px-3 py-2.5", rowCard)}>
                ...
                {finalStep === "photos" ? (
                  <div>
                    ...
                    >
                      {photos.length}/{GEN_MIN_PHOTOS} photos added — 2 are
                      required
                    </p>
                    <input
                      ref={photoRef}
            ...
            <div className="flex shrink-0 flex-col gap-2 pt-2">
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  disabled={busy || generatorSendDisabled || photosIncomplete}
                  onClick={() => void send()}
