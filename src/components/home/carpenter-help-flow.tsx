...
const URGENCY_CHIPS: {
  ...
  { id: "normal", label: CARPENTER_FINAL_COPY.normal, fee: "1x · base + call-out" },
  { id: "emergency", label: CARPENTER_FINAL_COPY.emergency, fee: "1.25x · base + call-out" },
  { id: "remote", label: CARPENTER_FINAL_COPY.remote, fee: "1.35x · base + call-out" },
  { id: "night", label: CARPENTER_FINAL_COPY.night, fee: "1.5x · base + call-out" },
];

type FinalStep = "urgency" | "photos" | "voice" | "location" | "measurements";

const FLOW_SESSION_KEY = "ona-carpenter-flow-session";
/** Guard against exceeding the ~5 MB sessionStorage quota with media data URLs. */
const FLOW_SESSION_MAX_BYTES = 3_000_000;

interface CarpenterFlowSnapshot {
  stack: string[];
  ...
  measurementsChoice: string | null;
}

export function CarpenterHelpFlow({
  isLight,
  onExit,
}: {
  isLight: boolean;
  ...
  const { urgency, setUrgency, restoreUrgency } = useAutoCalloutUrgency({
    unsafe: false,
    distanceKm: nearestProDistanceKm(visibleTechnicians, ["carpenter"]),
  });
  ...
  const [measurementsChoice, setMeasurementsChoice] = useState<string | null>(
    null
  );
  ...
  const screen = carpenterScreen(step);
  ...
  useEffect(() => {
    ...
    const snap = readSession<CarpenterFlowSnapshot>(FLOW_SESSION_KEY);
    if (snap) {
      setStack(
      ...
      setMeasurementsChoice(snap.measurementsChoice ?? null);
      setDir("fwd");
    }
    setHydrated(true);
  ...
  useEffect(() => {
    if (!hydrated) return;
    const snap: CarpenterFlowSnapshot = {
      stack,
      answers,
      draft,
      urgency,
  ...
  const pick = (optionId: string, label: string) => {
    ...
    push(nextCarpenterScreen(step, optionId, nextAnswers));
  };
  ...
  const submitText = () => {
    ...
    setAnswers(nextAnswers);
    setDraft("");
    push(nextCarpenterScreen(step, draft.trim(), nextAnswers));
  };
  ...
  const goBack = () => {
    ...
    if (carpenterScreen(leaving)?.kind === "text") {
      setDraft(answers[leaving] || "");
    } else {
      setDraft(
        answers[prev] && carpenterScreen(prev)?.kind === "text"
          ? answers[prev]
          : ""
      );
    }
  ...
  const onPhotos = async (files: FileList | null) => {
    ...
    const room = CARPENTER_MAX_PHOTOS - photos.length;
    if (room <= 0) {
    ...
    try {
      ...
      setPhotos((prev) => [...prev, ...next].slice(0, CARPENTER_MAX_PHOTOS));
      clearAdvanceTimer();
      if (total >= CARPENTER_MIN_PHOTOS) {
  ...
  const send = async () => {
    ...
    const measurementsLabel =
      CARPENTER_MEASUREMENTS_OPTIONS.find(
        (o) => o.id === measurementsChoice
      )?.label || measurementsChoice || "";
    const problem = [
      measurementsLabel
        ? `${CARPENTER_FINAL_COPY.measurements} ${measurementsLabel}`
        : "",
      composeCarpenterJob(answers, extra, landmark),
    ]
      .filter(Boolean)
      .join("\n");
    const payload = {
      ...
      serviceType: "carpenter" as const,
  ...
  const photosIncomplete = photos.length < CARPENTER_MIN_PHOTOS;
  const carpenterSendDisabled =
    finalStep === "measurements" && measurementsChoice === null;
          ...
          <div className="flex min-h-0 flex-1 flex-col">
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain scrollbar-hide">
              <p className={cn("mt-2 px-0.5 pb-2 text-[14px] font-bold capitalize leading-snug", ink)}>
                {finalStep === "urgency"
                  ...
                  : finalStep === "photos"
                    ? CARPENTER_FINAL_COPY.photos
                    : finalStep === "voice"
                      ? CARPENTER_FINAL_COPY.voice
                      : finalStep === "location"
                ...
                {finalStep === "photos" ? (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      {photos.length < CARPENTER_MAX_PHOTOS ? (
                        <button
                          type="button"
                  ...
                  <div>
                    ...
                    {measurementsChoice === null ? (
                      ...
                      <div className="mt-2 flex items-center justify-between gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => setMeasurementsChoice(null)}
                          className={cn(
            ...
            <div className="flex shrink-0 flex-col gap-2 pt-2">
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  disabled={busy || carpenterSendDisabled || photosIncomplete}
                  onClick={() => void send()}
