...
describe("generator question tree", () => {
  ...
  it("walks the full purchase (A) and servicing (D) chains to final", () => {
    ...
    expect(nextGeneratorScreen("a_size", "5-10", {})).toBe("a_sound");
    expect(nextGeneratorScreen("a_sound", "soundproof", {})).toBe("a_supply");
    expect(nextGeneratorScreen("a_supply", "i-will", {})).toBe(
      "a_changeover"
    );
    expect(nextGeneratorScreen("a_changeover", "automatic", {})).toBe(
      "final"
    );
  ...
  it("walks the will-not-start (B), no-power (C), change-over (E) and other (G) chains to confirm", () => {
    ...
    expect(nextGeneratorScreen("c_voltage", "fluctuating", {})).toBe(
      "c_load"
    );
    ...
    expect(nextGeneratorScreen("e_history", "new install", {})).toBe(
      "confirm"
    );
