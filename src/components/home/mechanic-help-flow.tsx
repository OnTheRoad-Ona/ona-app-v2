...
const URGENCY_CHIPS: {
  ...
  {
    id: "normal",
    label: MECHANIC_FINAL_COPY.normal,
    fee: "1x · base + call-out",
  },
  {
    id: "emergency",
    label: MECHANIC_FINAL_COPY.emergency,
    fee: "1.25x · base + call-out",
  },
  {
    id: "remote",
    label: MECHANIC_FINAL_COPY.remote,
    fee: "1.35x · base + call-out",
  },
  {
    id: "night",
    label: MECHANIC_FINAL_COPY.night,
...
export function MechanicHelpFlow({
  ...
  const { urgency, setUrgency, restoreUrgency } = useAutoCalloutUrgency({
    unsafe: ["c_safe", "d_safe", "f_safe"].some((k) => answers[k] === "no"),
  ...
  const goBack = () => {
    ...
    if (mechanicScreen(leaving)?.kind === "text") {
      ...
      setDraft(
        answers[prev] && mechanicScreen(prev)?.kind === "text"
          ? answers[prev]
          : "",
      );
    }
