...
function effectiveOf(p: Payment): string {
  const esc = String(p.escrow_status || p.status || "")
    .toLowerCase()
    .trim();
  ...
  if (payout === "suspended_admin" || meta.payoutSuspended === true)
    return "suspended";
  if (esc === "failed" || payout === "failed" || meta.payoutFailedAt)
    return "failed";
...
export default function PaymentControlCenter() {
  ...
  const [disputeOutcome, setDisputeOutcome] =
    useState<string>("full_release_pro");
  ...
  const [commReport, setCommReport] = useState<Record<string, unknown> | null>(
    null,
  );
  ...
  const load = useCallback(async () => {
    const res = await api<{
      ...
      access?: { canCancelEscrow?: boolean; canRetryPayout?: boolean };
    }>("/api/admin/payments");
    setLoading(false);
    if (!res.ok) {
  ...
  const loadDisputes = useCallback(async () => {
    const res = await api<{ jobs: Record<string, unknown>[] }>(
      "/api/admin/disputes",
    );
  ...
  const loadAudit = useCallback(async () => {
    const res = await api<{ actions: Record<string, unknown>[] }>(
      "/api/admin/audit",
    );
    if (res.ok)
      setAuditLog(Array.isArray(res.data.actions) ? res.data.actions : []);
  }, [api]);
  ...
  const loadFailedPayouts = useCallback(async () => {
    const res = await api<{
      ...
      totalCount: number;
    }>("/api/admin/failed-payouts");
    if (res.ok) {
      setFailedPayouts(Array.isArray(res.data.failed) ? res.data.failed : []);
      setFailedPayoutsTotal(res.data.totalCount);
    }
  }, [api]);
  ...
  const loadCommission = useCallback(
    async (period: string) => {
      ...
      const res = await api<Record<string, unknown>>(
        `/api/admin/commission?period=${p}&from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`,
      );
      if (res.ok) {
        setCommReport(res.data);
        setCommYtd(
          ((res.data as Record<string, unknown>).ytd as Record<
            string,
            unknown
          >) || null,
        );
  ...
  useEffect(() => {
    if (!ready || tab !== "disputes") return;
    void loadDisputes();
  }, [ready, tab, loadDisputes]);
  useEffect(() => {
    if (!ready || tab !== "audit") return;
    void loadAudit();
  }, [ready, tab, loadAudit]);
  useEffect(() => {
    if (!ready || tab !== "failed") return;
    void loadFailedPayouts();
  }, [ready, tab, loadFailedPayouts]);
  useEffect(() => {
    if (!ready || tab !== "commission") return;
    void loadCommission(commPeriod);
  }, [ready, tab, loadCommission, commPeriod]);
  ...
  async function withPassword(action: () => Promise<void>) {
    ...
    setBusy(true);
    setError(null);
    const unlock = await api<{ expiresAt?: string }>("/api/admin/care/unlock", {
      method: "POST",
      body: JSON.stringify({ password: password.trim() }),
    });
    if (!unlock.ok) {
      setError(unlock.message || "Invalid code");
      setBusy(false);
      setPassword("");
      return;
    }
  ...
  const filtered = useMemo(() => {
    ...
    return payments.filter((p) => {
      ...
      if (filter !== "all") {
        if (filter === "held") {
          if (e !== "held" && e !== "pending_payment") return false;
        } else if (filter === "pending") {
          if (
            e !== "pending" &&
            e !== "release_pending" &&
            e !== "pending_settlement"
          )
            return false;
  ...
  const esc = String(
    detail?.payment
      ? (detail.payment as Record<string, unknown>).escrow_status
      : "",
  ).toLowerCase();
  const meta = (
    detail?.payment
      ? (detail.payment as Record<string, unknown>).meta || {}
      : {}
  ...
  const parties = detail?.parties as
    | {
  ...
  const flwTransfers = (detail?.flwTransfers || []) as {
    id?: string;
    reference?: string;
    status?: string;
    amount?: number;
  ...
  const s: Record<string, React.CSSProperties> = {
    ...
    th: {
      ...
      padding: "8px 10px",
      borderBottom: "2px solid var(--om-border)",
      color: "var(--om-text-muted)",
      fontSize: 10,
    ...
    ok: {
      ...
      border: "1px solid var(--om-success)",
      fontSize: 13,
      marginBottom: 12,
    },
  };
  ...
  const summaryCards = [
    {
      ...
      val: ops
        ? `₦${((ops.ona.escrowHeldMinor + ops.ona.pendingSettlementMinor) / 100).toLocaleString("en-NG", { minimumFractionDigits: 2 })}`
        : "",
    ...
    },
    {
      label: "FLW Available",
      val: ops
        ? `₦${(ops.flw.available ?? 0).toLocaleString("en-NG", { minimumFractionDigits: 2 })}`
        : "",
      sub: "Payout balance",
    },
    {
      label: "FLW Ledger",
      val: ops
        ? `₦${(ops.flw.ledger ?? 0).toLocaleString("en-NG", { minimumFractionDigits: 2 })}`
        : "",
      sub: "Settling",
    },
    {
      label: "Escrow held",
      val: ops ? naira(ops.ona.escrowHeldMinor) : "",
      sub: "Customer paid",
    },
    {
      label: "Pending settlement",
      val: ops ? naira(ops.ona.pendingSettlementMinor) : "",
      sub: `${ops?.ona.pendingSettlementCount ?? 0} jobs`,
    },
    {
      label: "Commission earned",
      val: ops ? naira(ops.ona.totalCommissionEarnedMinor) : "",
      sub: `${ops?.ona.commissionEarnedCount ?? 0} paid jobs`,
    },
    {
      label: "Paid to pros",
      val: String(ops?.ona.releasedCount ?? ""),
      sub: "Released",
    },
    {
      label: "Failed",
      val: String(ops?.ona.failedCount ?? ""),
      sub: `${ops?.ona.exhaustedCount ?? 0} exhausted`,
    },
    {
      label: "Suspended",
      val: String(ops?.ona.suspendedCount ?? ""),
      sub: "Admin stopped",
    },
    {
      label: "Refunded",
      val: String(ops?.ona.refundedCount ?? ""),
      sub: "Customer refunded",
    },
    {
      label: "Disputed",
      val: String(ops?.ona.disputedCount ?? ""),
      sub: "In dispute",
    },
  ];
  ...
  return (
    <AdminShell adminName={adminName} adminRole={adminRole}>
      ...
      <p style={{ ...s.muted, marginBottom: 16, lineHeight: 1.5 }}>
        Financial control room: monitor, control, release, refund, cancel,
        dispute, and audit every transaction. Sensitive actions require
        temporary access code. <strong>All actions are logged.</strong>
      </p>
      ...
      {showPassword && (
        ...
        >
          ...
          >
            Sensitive action requires temporary access code
          </p>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && pendingAction)
                  void withPassword(pendingAction);
              }}
              autoFocus
            />
            <button
              ...
              disabled={busy}
              onClick={() => {
                if (pendingAction) void withPassword(pendingAction);
              }}
            >
              Confirm
            </button>
      ...
      >
        {(
          ["payments", "disputes", "audit", "failed", "commission"] as TabKey[]
        ).map((t) => (
          <button
            key={t}
            style={{
              ...
              background:
                tab === t ? "var(--om-nav-active)" : "var(--om-panel-2)",
              borderColor: tab === t ? "var(--om-role)" : "var(--om-border)",
            }}
            onClick={() => {
              setTab(t);
              setSelectedId(null);
        ...
        <>
          ...
          >
            {summaryCards.map((c) => (
              <div
                ...
                style={s.card}
                onClick={() => {
                  const k = c.label.toLowerCase().includes("held")
                    ? "held"
                    : c.label.toLowerCase().includes("pending")
                      ? "pending"
                      : c.label.toLowerCase().includes("paid") ||
                          c.label.toLowerCase().includes("released")
                        ? "released"
                        : c.label.toLowerCase().includes("fail")
                          ? "failed"
                          : c.label.toLowerCase().includes("refund")
                            ? "refunded"
          ...
          >
            <input
              ...
              placeholder="Search id, ref, customer, pro…"
              value={search}
          ...
          <div style={s.panel}>
              ...
              <table style={s.table}>
                ...
                <tbody>
                  {loading && payments.length === 0 ? (
                    <tr>
                      <td colSpan={11} style={{ ...s.td, ...s.muted }}>
                        Loading...
                      </td>
                    </tr>
                  ) : filtered.length === 0 ? (
                    <tr>
                      <td colSpan={11} style={{ ...s.td, ...s.muted }}>
                        No payments match <strong>{filter}</strong> with the
                    ...
                    filtered.map((pm) => {
                      ...
                      return (
                        <tr
                          key={String(pm.id)}
                          style={{
                            background: sel ? "var(--om-nav-hover)" : undefined,
                            cursor: "pointer",
                          }}
                          onClick={() =>
                            setSelectedId(sel ? null : String(pm.id))
                          }
                        >
                          <td
                            style={{
                          ...
                          >
                            {String(pm.id).slice(0, 8)}
                          </td>
                          <td style={s.td}>
                            <strong>{naira(amt)}</strong>
                          </td>
                          <td style={s.td}>
                            <span style={badgeStyle(e)}>
                              {String(pm.display_status || e)}
                            </span>
                          </td>
                          <td
                            style={{
                          ...
                          >
                            {String(
                              pm.motorist_name || pm.motoristId || "",
                            ).slice(0, 12) || ""}
                          </td>
                          <td
                            style={{
                          ...
                          >
                            {String(
                              pm.repair_pro_name || pm.repairProId || "",
                            ).slice(0, 12) || ""}
                          </td>
                          <td style={{ ...s.td, ...s.muted, fontSize: 10 }}>
                            {String(pm.service_type || "").slice(0, 10)}
                          </td>
                          <td style={s.td}>{fee ? naira(fee) : ""}</td>
                          <td style={s.td}>
                            {proShare ? naira(proShare) : ""}
                          </td>
                          <td style={s.td}>
                            {String(
                              pm.payout_status ||
            ...
            {selectedId && (
              ...
              >
                {detailLoading ? (
                  <p style={s.muted}>Loading detail...</p>
                ) : !detail ? (
                  ...
                  <>
                    ...
                    >
                      ...
                      >
                        ...
                        <div
                          style={{ fontWeight: 700, color: "var(--om-text)" }}
                        >
                        ...
                        <div style={{ marginTop: 4 }}>
                          Paid{" "}
                          <strong>{naira(Number(p?.amount_kobo || 0))}</strong>
                        </div>
                      </div>
                      <div
                        style={{
                      ...
                      >
                        ...
                        <div style={s.muted}>
                          {parties?.pro?.bank?.bankName || ""} ·{" "}
                          {parties?.pro?.bank?.accountName || ""} · ****
                          {parties?.pro?.bank?.accountLast4 || "????"}
                        </div>
                        <div style={{ marginTop: 4 }}>
                          Pro share{" "}
                          <strong>
                            {p?.pro_payout_kobo != null
                              ? naira(Number(p.pro_payout_kobo))
                              : ""}
                          </strong>
                        </div>
                      </div>
                    ...
                    >
                      <span style={s.muted}>Escrow status</span>
                      <span style={{ color: "var(--om-text)" }}>
                        <span style={badgeStyle(esc)}>{esc}</span> ·{" "}
                        {String(p?.status)}
                      </span>
                      <span style={s.muted}>Payment ID</span>
                      <span
                        style={{
                          wordBreak: "break-all",
                          fontSize: 10,
                      ...
                      <span style={s.muted}>Job / request</span>
                      <span
                        style={{
                          wordBreak: "break-all",
                          fontSize: 10,
                      ...
                      <span style={s.muted}>FLW ref</span>
                      <span style={{ fontSize: 10, color: "var(--om-text)" }}>
                        {String(p?.provider_ref || "")}
                      </span>
                      <span style={s.muted}>Payout ref</span>
                      <span style={{ fontSize: 10, color: "var(--om-text)" }}>
                        {String(
                          detail?.idempotent_transfer_ref ||
                            meta.idempotentTransferRef ||
                            "",
                        )}
                      </span>
                      <span style={s.muted}>Last error</span>
                      <span style={{ color: "var(--om-danger)" }}>
                        {String(meta.lastReleaseError || "")}
                      </span>
                      <span style={s.muted}>Retries</span>
                      <span>
                      ...
                      <span style={s.muted}>Ona commission</span>
                      <span>
                        {p?.platform_fee_kobo != null
                          ? naira(Number(p.platform_fee_kobo))
                          : ""}
                      </span>
                      <span style={s.muted}>Pro payout</span>
                      <span>
                        {p?.pro_payout_kobo != null
                          ? naira(Number(p.pro_payout_kobo))
                          : ""}
                      </span>
                      {detail?.doublePayRisk ? (
                        <span
                          style={{
                            color: "var(--om-danger)",
                            gridColumn: "1 / -1",
                    ...
                    {alreadyPaid && (
                      <div style={s.ok}>
                        Pro payout already completed. Do not force a second
                        transfer.
                      </div>
                    )}
                    ...
                    {!detail?.parties && (
                      <p style={{ ...s.muted, fontSize: 11 }}>
                        Full party details require view_payment_full permission.
                      </p>
                    )}
                    ...
                    <div style={{ marginBottom: 8 }}>
                      ...
                      >
                        Reason note (required for sensitive actions):
                      </p>
                      <textarea
                        style={s.ta}
                        rows={2}
                        placeholder="e.g. Force payout, customer refund..."
                        value={reason}
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                      ...
                      {canCancel && (
                        <button
                          ...
                          disabled={
                            busy || esc === "released" || esc === "refunded"
                          }
                        >
                          Refund customer
                        </button>
                    ...
                    {!canForce && !canCancel && (
                      ...
                      >
                        Read-only view. Force/refund requires L3+.
                      </p>
                    )}
                          ...
                          <tbody>
                            {ledger.map((r) => (
                              <tr key={r.id}>
                                ...
                                <td
                                  style={{ ...s.td, ...s.muted, fontSize: 10 }}
                                >
                                  {r.createdAt
              ...
              >
                {modal === "force" && (
                  <>
                    ...
                    <p style={s.muted}>
                      Bypasses standard flow. Same transfer ref no double pay.
                      Requires L4+.
                    </p>
                    ...
                    <p style={{ fontSize: 13, margin: "12px 0" }}>
                      Amount:{" "}
                      <strong>
                        {p ? naira(Number(p.pro_payout_kobo || 0)) : ""}
                      </strong>
                    </p>
                    <p style={{ fontSize: 13 }}>
                      To: <strong>{parties?.pro?.name || ""}</strong> ·{" "}
                      {parties?.pro?.bank?.bankName || ""} ****
                      {parties?.pro?.bank?.accountLast4 || "????"}
                    </p>
                    <div
                    ...
                    >
                      <button style={s.btn} onClick={() => setModal(null)}>
                        Cancel
                      </button>
                      <button
                        style={s.btnP}
                ...
                {modal === "refund" && (
                  <>
                    ...
                    <p style={s.muted}>
                      Cancel escrow and mark refunded. Funds returned to
                      customer.
                    </p>
                    <p style={{ fontSize: 13, margin: "12px 0" }}>
                      Reason: <strong>{reason}</strong>
                    ...
                    <p style={{ fontSize: 13 }}>
                      Amount:{" "}
                      <strong>
                        {p ? naira(Number(p.amount_kobo || 0)) : ""}
                      </strong>
                    </p>
                    <div
                      style={{
                ...
                {modal === "retry" && (
                  <>
                    ...
                    <p style={s.muted}>
                      Idempotent retry. Respects 10-min spacing unless already
                      due.
                    </p>
                    <p style={{ fontSize: 13, margin: "12px 0" }}>
                      Reason: <strong>{reason}</strong>
                ...
                {modal === "stop" && (
                  <>
                    ...
                    <p style={s.muted}>
                      Stops auto-retry. Funds stay held in escrow (not a
                      refund).
                    </p>
                    <p style={{ fontSize: 13, margin: "12px 0" }}>
                      Reason: <strong>{reason}</strong>
      ...
      {tab === "disputes" && (
        <div style={s.panel}>
          ...
          {disputes.length === 0 ? (
            <p style={s.muted}>No disputed transactions.</p>
          ) : (
            <>
              <div
                style={{
                  maxHeight: disputeDetailId ? 240 : 500,
                  overflowY: "auto",
                  marginBottom: 8,
                }}
              >
                    ...
                    {disputes.map((j) => {
                      ...
                      return (
                        <tr
                          key={String(j.id)}
                          style={{
                            background: sel ? "var(--om-warn-bg)" : undefined,
                            cursor: "pointer",
                          }}
                          onClick={() =>
                            setDisputeDetailId(sel ? null : String(j.id))
                          }
                        >
                          <td style={s.td}>
                          ...
                          <td style={{ ...s.td, fontSize: 11 }}>
                            <div>
                              {String(j.motorist_name || "").slice(0, 14)}
                            </div>
                            <div style={s.muted}>
                              {String(j.repair_pro_name || "").slice(0, 14)}
                            </div>
                          </td>
                          <td style={s.td}>
                            <span style={badgeStyle(String(j.status || ""))}>
                              {String(j.status || "")}
                            </span>
                            {ev.priority === "auto_priority" ? (
                              <div style={{ ...s.muted, fontSize: 9 }}>
                                AI priority
                              </div>
                            ) : null}
                            {ev.priority === "request_more" ? (
                              <div style={{ ...s.muted, fontSize: 9 }}>
                                Need evidence
                              </div>
                            ) : null}
                          ...
                          >
                            ...
                            {d.description ? (
                              ...
                              >
                                {(d.description as string).slice(0, 40)}
                              </div>
                            ) : null}
                          </td>
                          <td style={s.td}>
                            ...
                            {ev.photoScore != null ? (
                              <div style={{ ...s.muted, fontSize: 9 }}>
                                P{Number(ev.photoScore)} V
                                {Number(ev.voiceScore)} L
                                {Number(ev.locationScore)} T
                                {Number(ev.timestampScore)}
                              </div>
                            ) : null}
                          </td>
                          <td style={s.td}>
                            {j.agreed_major != null
                              ? `₦${Number(j.agreed_major).toLocaleString("en-NG")}`
                              : ""}
                          </td>
                          <td style={s.td}>
                            <div
                            ...
                            >
                              ...
                              {disputeOutcome === "partial_split" && (
                                <input
                                  ...
                                  min={0}
                                  max={100}
                                  value={splitPct[String(j.id)] ?? 70}
                                  onChange={(e) =>
                                    setSplitPct((p) => ({
                                      ...p,
                                      [String(j.id)]: Number(e.target.value),
                                    }))
                                  }
                                  style={{
                                    width: 44,
              ...
              {disputeDetailId &&
                (() => {
                  const j = disputes.find(
                    (d) => String(d.id) === disputeDetailId,
                  );
                  if (!j) return null;
                  const d = (j.dispute || {}) as Record<string, unknown>;
                  const ev = (j.evidence || {}) as Record<string, unknown>;
                  const decision = (d.decision || {}) as Record<
                    string,
                    unknown
                  >;
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        Dispute Detail {String(j.id).slice(0, 12)}
                      </h3>
                      <div
                        style={{
                          display: "grid",
                      ...
                      >
                        <span style={s.muted}>Opened by</span>
                        <span style={{ color: "var(--om-text)" }}>
                          {String(d.openedBy || "")}
                        </span>
                        <span style={s.muted}>Opened at</span>
                        <span>
                          {d.openedAt
                            ? new Date(String(d.openedAt)).toLocaleString()
                            : ""}
                        </span>
                        <span style={s.muted}>Reason</span>
                        ...
                        {ev.composite != null && (
                          <>
                            ...
                            <span>
                              <strong>{Number(ev.composite)}</strong>/100 &nbsp;
                              P{Number(ev.photoScore)} V{Number(ev.voiceScore)}{" "}
                              L{Number(ev.locationScore)} T
                              {Number(ev.timestampScore)}
                            </span>
                          </>
                        )}
                        {((ev.flags as string[]) || []).length > 0 && (
                          <>
                            <span style={s.muted}>Flags</span>
                            <span style={s.muted}>
                              {String((ev.flags as string[]).join(", "))}
                            </span>
                          </>
                        )}
                        {(d.media as unknown[]) &&
                          Array.isArray(d.media) &&
                          (d.media as unknown[]).length > 0 && (
                            <>
                              <span style={s.muted}>Media</span>
                              <span>
                                {String((d.media as unknown[]).length)} file(s)
                              </span>
                            </>
                          )}
                      </div>
                      ...
                      {Object.keys(decision).length > 0 && (
                        ...
                        >
                          ...
                          >
                            <span style={s.muted}>Outcome</span>
                            <span>
                              <strong>{String(decision.outcome || "")}</strong>
                            </span>
                            {decision.proPercent != null && (
                              <span style={s.muted}>Split</span>
                            )}
                            <span>
                              {decision.proPercent != null
                                ? `Pro ${Number(decision.proPercent)}% / Cust ${Number(decision.motoristPercent || 0)}%`
                                : ""}
                            </span>
                            {decision.note ? (
                              <>
                                <span style={s.muted}>Note</span>
                                <span>{String(decision.note)}</span>
                              </>
                            ) : null}
                            <span style={s.muted}>Decided at</span>
                            <span>
                              {decision.decidedAt
                                ? new Date(
                      ...
                      {Object.keys(appeal).length > 0 && (
                        ...
                        >
                          ...
                          >
                            <span style={s.muted}>By</span>
                            <span style={{ color: "var(--om-text)" }}>
                              {String(appeal.openedBy || "")}
                            </span>
                            <span style={s.muted}>Reason</span>
                            <span style={{ color: "var(--om-text)" }}>
                            ...
                            <span style={{ color: "var(--om-text)" }}>
                              {appeal.openedAt
                                ? new Date(
                                    String(appeal.openedAt),
                                  ).toLocaleString()
                                : ""}
                            </span>
                            {(appeal.media as unknown[]) &&
                            Array.isArray(appeal.media) &&
                            (appeal.media as unknown[]).length > 0 ? (
                              <>
                                <span style={s.muted}>Media</span>
                                <span style={{ color: "var(--om-text)" }}>
                                  {String((appeal.media as unknown[]).length)}{" "}
                                  file(s)
                                </span>
                              </>
                            ) : null}
                          ...
                          {(appeal.decision || {}) &&
                            Object.keys(appeal.decision || {}).length > 0 && (
                              <div
                                style={{
                                  marginTop: 6,
                                  padding: 8,
                      ...
                      <p style={{ ...s.muted, fontSize: 10 }}>
                        Click the row to dismiss this detail panel.
                      </p>
                    </div>
                  );
                })()}
      ...
      {tab === "audit" && (
        <div style={s.panel}>
          ...
          {auditLog.length === 0 ? (
            <p style={s.muted}>No audit records found.</p>
          ) : (
                  ...
                  {auditLog.map((a, i) => {
                    ...
                    return (
                      <tr key={String(a.id || i)}>
                        ...
                        <td style={{ ...s.td, fontSize: 11 }}>
                          {String(a.admin_id || "").slice(0, 8) ||
                            String(a.admin_name || "")}
                        </td>
                        <td style={s.td}>
                          <span style={badgeStyle("held")}>
                            {String(a.action || "")}
                          </span>
                        </td>
                        <td style={{ ...s.td, ...s.muted, fontSize: 10 }}>
                          {String(a.target_user_id || a.target_id || "").slice(
                            0,
                            8,
                          ) || ""}
                        </td>
                        ...
                        >
                          {String(
                            actMeta.reason || actMeta.note || a.details || "",
                          ).slice(0, 60)}
                        </td>
                        <td style={s.td}>
                          {a.error ? (
                            ...
                            <span style={{ color: "var(--om-success)" }}>
                              Success
                            </span>
                          )}
                        </td>
        ...
        <div style={s.panel}>
          ...
          >
            <h2 style={{ fontSize: 15, fontWeight: 600, margin: 0 }}>
              Failed Payouts ({failedPayoutsTotal})
            </h2>
            <div style={{ display: "flex", gap: 6 }}>
              <button
                style={s.btn}
                onClick={() => void loadFailedPayouts()}
                disabled={busy}
              >
                Refresh
              </button>
              <button
                style={{ ...s.btnP, fontSize: 10 }}
                onClick={async () => {
                  setBusy(true);
                  const res = await api("/api/admin/payments", {
                    method: "PATCH",
                    body: JSON.stringify({ action: "retry_all_due" }),
                  });
                  setBusy(false);
                  if (!res.ok) {
                    setError(res.message);
          ...
          {failedPayouts.length === 0 ? (
            <p style={s.muted}>No failed payouts. All clear.</p>
          ) : (
                ...
                <tbody>
                  {failedPayouts.map((fp) => {
                    ...
                    return (
                      <tr key={String(fp.id)}>
                        <td style={{ ...s.td, ...s.muted, fontSize: 10 }}>
                          {String(fp.requestId || "").slice(0, 12) ||
                            String(fp.id).slice(0, 12)}
                        </td>
                        <td style={s.td}>
                          {naira(Number(fp.amountMinor || 0))}
                        </td>
                        <td style={s.td}>{String(fp.retryCount ?? "")}</td>
                        <td style={s.td}>
                          {isExhausted ? (
                            <span style={badgeStyle("failed")}>Exhausted</span>
                          ) : isSuspended ? (
                            <span style={badgeStyle("cancelled")}>
                              Suspended
                            </span>
                          ) : (
                        ...
                        <td style={{ ...s.td, ...s.muted, fontSize: 10 }}>
                          {fp.nextRetryAt
                            ? new Date(String(fp.nextRetryAt)).toLocaleString()
                            : ""}
                        </td>
                        <td style={s.td}>
                          <div
            ...
            <div style={{ display: "flex", gap: 6 }}>
              {(["day", "week", "month", "year", "ytd"] as const).map((p) => (
                <button
                  ...
                  style={{
                    ...s.btn,
                    fontWeight: commPeriod === p ? 700 : 400,
                    fontSize: 10,
                  }}
                  onClick={() => {
                    setCommPeriod(p);
              ...
              >
                <div style={s.card}>
                  ...
                  <div style={s.cardVal}>
                    {naira(
                      Number(
                        (commReport as Record<string, unknown>)
                ...
                <>
                  ...
                  >
                    <div style={s.card}>
                      ...
                      <div style={s.cardVal}>
                        {naira(Number(commYtd.totalCommissionMinor || 0))}
                      </div>
                    </div>
                    <div style={s.card}>
                      <div style={s.cardLabel}>YTD Revenue</div>
                      <div style={s.cardVal}>
                        {naira(Number(commYtd.totalRevenueMinor || 0))}
                      </div>
                    </div>
                    <div style={s.card}>
                      <div style={s.cardLabel}>YTD Payout</div>
                      <div style={s.cardVal}>
                        {naira(Number(commYtd.totalPayoutMinor || 0))}
                      </div>
                    </div>
                  </div>
                </>
                ...
                <table style={s.table}>
                  ...
                  <tbody>
                    {(
                      ...
                      <tr>
                        <td colSpan={5} style={{ ...s.td, ...s.muted }}>
                      ...
                      (
                        ...
                        <tr key={String(b.date || i)}>
                          <td style={{ ...s.td, ...s.muted, fontSize: 10 }}>
                            {commPeriod === "year"
                              ? String(b.date || "").slice(0, 4)
                              : String(b.date || "").slice(0, 10)}
                          </td>
                          <td style={s.td}>{String(b.count || 0)}</td>
                          <td style={s.td}>
                            {naira(Number(b.revenueMinor || 0))}
                          </td>
                          <td style={s.td}>
                            <strong>
                              {naira(Number(b.commissionMinor || 0))}
                            </strong>
                          </td>
                          <td style={s.td}>
                            {naira(Number(b.payoutMinor || 0))}
                          </td>
                        </tr>
                      ))
                    )}
...
function ManualPayoutForm({
  ...
  async function submit() {
    if (
      ...
      setErr(
        "All fields required: amount, bank code, 10-digit account, name, reason (8+ chars)",
      );
      return;
    }
    setBusy(true);
  ...
  return (
    <>
      ...
      <p style={muted}>
        Not tied to a job. Unique ref prevents double pay. L4+.
      </p>
      {err ? (
        <div
          style={{ color: "var(--om-danger)", fontSize: 12, marginBottom: 8 }}
        >
          {err}
        </div>
      ) : null}
      ...
      >
        ...
        <button style={btnP} onClick={submit} disabled={busy}>
          Submit Payout
        </button>
      </div>
    </>
