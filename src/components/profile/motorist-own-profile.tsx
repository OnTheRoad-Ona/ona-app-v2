...
export function MotoristOwnProfile({ isLight }: { isLight: boolean }) {
  ...
  const { userProfile, updateUserProfile, location } = useApp();
  ...
  const [saved, setSaved] = useState(userProfile?.savedLocations || []);

  const jobsRequested =
    userProfile?.jobsRequested ?? userProfile?.serviceActionCount ?? 0;
  ...
  if (!userProfile || userProfile.accountType !== "motorist") {
    return (
      <ProfileShell
        isLight={isLight}
        title="My Profile"
        error="Sign in as a Customer to view this profile."
      />
    );
  }
        ...
        action={
          !addingVehicle ? (
            <button
              ...
              className={cn(
                "inline-flex items-center gap-1 border-0 bg-transparent text-[12px] font-bold",
                isLight ? "text-slate-900" : "text-white",
              )}
            >
        ...
        <div className="space-y-3">
          {vehicles.length === 0 && !addingVehicle && (
            <p className={cn("text-[12px]", t.muted)}>
              Add at least one vehicle. You can add as many as you need one step
              at a time.
            </p>
          )}
          ...
          {vehicles.map((v) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1 space-y-0.5 text-[12px]">
                ...
                {v.plate ? <p className={t.muted}>Plate · {v.plate}</p> : null}
            ...
            {saved.map((loc) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className={cn("truncate text-[11px]", t.muted)}>
                    {loc.address}
                  </p>
                </div>
                {editing && (
