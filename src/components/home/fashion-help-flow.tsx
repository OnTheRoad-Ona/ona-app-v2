...
interface FashionFlowSnapshot {
  ...
  homeChoice: "home" | "visit" | null;
}

export function FashionHelpFlow({
  isLight,
  onExit,
}: {
  isLight: boolean;
  ...
  const [homeChoice, setHomeChoice] = useState<"home" | "visit" | null>(null);
  ...
  const screen = fashionScreen(step);
  ...
  useEffect(() => {
    ...
    if (snap) {
      ...
      setHomeChoice(snap.homeChoice ?? null);
      setDir("fwd");
    }
    setHydrated(true);
  ...
  useEffect(() => {
    if (!hydrated) return;
    const snap: FashionFlowSnapshot = {
      stack,
      answers,
      draft,
      urgency,
  ...
  const pick = (optionId: string, label: string) => {
    ...
    push(nextFashionScreen(step, optionId, nextAnswers));
  };
  ...
  const submitText = () => {
    ...
    setAnswers(nextAnswers);
    setDraft("");
    push(nextFashionScreen(step, draft.trim(), nextAnswers));
  };
  ...
  const goBack = () => {
    ...
    if (fashionScreen(leaving)?.kind === "text") {
  ...
  const onPhotos = async (files: FileList | null) => {
    ...
    const room = FASHION_MAX_PHOTOS - photos.length;
    if (room <= 0) {
    ...
    try {
      ...
      setPhotos((prev) => [...prev, ...next].slice(0, FASHION_MAX_PHOTOS));
      clearAdvanceTimer();
      if (total >= FASHION_MIN_PHOTOS) {
  ...
  const send = async () => {
    ...
    const problem = [
      homeChoice === "home"
        ? "Home service required: Yes"
        : "Visit workshop: Yes",
      composeFashionProblem(answers, extra, landmark),
    ]
      .filter(Boolean)
      .join("\n");
    const payload = {
          ...
          <div className="flex min-h-0 flex-1 flex-col">
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain scrollbar-hide">
              <div className="mt-2 flex items-center gap-1 px-0.5 pb-2">
                ...
                {finalStep === "urgency"
                  ...
                  : finalStep === "photos"
                    ? FASHION_FINAL_COPY.photos
                    : finalStep === "voice"
                      ? FASHION_FINAL_COPY.voice
                      : finalStep === "location"
                        ? FASHION_FINAL_COPY.location
                ...
                {finalStep === "photos" ? (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      {photos.length < FASHION_MAX_PHOTOS ? (
                        <button
                          type="button"
            ...
            <div className="flex shrink-0 flex-col gap-2 pt-2">
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  disabled={busy || homeSendDisabled || photosIncomplete}
                  onClick={() => void send()}
