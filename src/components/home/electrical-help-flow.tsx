...
interface ElectricalFlowSnapshot {
  ...
  towChoice: "tow" | "electrical" | null;
  towConfirming: boolean;
}

export function ElectricalHelpFlow({
  isLight,
  onExit,
}: {
  isLight: boolean;
  ...
  const [towChoice, setTowChoice] = useState<"tow" | "electrical" | null>(null);
  ...
  const screen = electricalScreen(step);
  ...
  useEffect(() => {
    if (!hydrated) return;
    const snap: ElectricalFlowSnapshot = {
      stack,
      answers,
      vehicleLabel,
      manualVehicles,
  ...
  const pick = (optionId: string, label: string) => {
    ...
    push(nextElectricalScreen(step, optionId, nextAnswers));
  };
  ...
  const submitText = () => {
    ...
    setAnswers(nextAnswers);
    setDraft("");
    push(nextElectricalScreen(step, draft.trim(), nextAnswers));
  };
  ...
  const goBack = () => {
    ...
    if (electricalScreen(leaving)?.kind === "text") {
  ...
  const onPhotos = async (files: FileList | null) => {
    ...
    const room = ELECTRICAL_MAX_PHOTOS - photos.length;
    if (room <= 0) {
    ...
    try {
      ...
      setPhotos((prev) => [...prev, ...next].slice(0, ELECTRICAL_MAX_PHOTOS));
      clearAdvanceTimer();
              ...
              <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain scrollbar-hide">
                <p className={cn("mt-2 px-0.5 pb-2 text-[14px] font-bold capitalize leading-snug", ink)}>
                  {finalStep === "urgency"
                    ...
                    : finalStep === "photos"
                      ? ELECTRICAL_FINAL_COPY.photos
                      : finalStep === "voice"
                        ? ELECTRICAL_FINAL_COPY.voice
                        : finalStep === "location"
                  ...
                  {finalStep === "photos" ? (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        {photos.length < ELECTRICAL_MAX_PHOTOS ? (
                          <button
                            type="button"
