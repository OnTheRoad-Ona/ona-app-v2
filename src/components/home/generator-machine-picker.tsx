...
import {
  GEN_MACHINE_NONE_ID,
  GEN_MACHINE_OTHER_ID,
} from "@/lib/generator/question-tree";
import { cn } from "@/lib/utils";

export type GeneratorMachineAnswers = {
  machine: string;
...
function SearchSelect({
  ...
  const filtered = useMemo(
    () => filterGeneratorOptions(options, open ? query : value, 80),
...
export function GeneratorMachinePicker({
  isLight,
  question,
  initial,
  onConfirm,
  ...
  onConfirm: (partial: GeneratorMachineAnswers) => void;
}) {
  ...
  const [otherMode, setOtherMode] = useState(
    initial.machine === GEN_MACHINE_OTHER_ID,
  );
  const [other, setOther] = useState(initial.machine_other || "");

  const brands = useMemo(() => getBrandsForType(typeId), [typeId]);
  const models = useMemo(
    () => getModelsForTypeBrand(typeId, brand),
    [typeId, brand],
  );
  ...
  const emitCatalog = (
    ...
    const typeLabel =
      GEN_CATALOG_TYPES.find((t) => t.id === nextType)?.label || nextType;
    const fuel = inferGeneratorFuel(nextType);
    onConfirm({
      machine: "catalog",
      machine_label: composeGeneratorMachineLabel({
        mode: "catalog",
        typeId: nextType,
        brand: nextBrand,
        model: nextModel,
  ...
  const emitNone = () => {
    onConfirm({
      machine: GEN_MACHINE_NONE_ID,
      machine_label: composeGeneratorMachineLabel({ mode: "none" }),
      machine_type: "",
      machine_type_label: "",
      machine_brand: "",
      machine_model: "",
    ...
    <div>
      ...
      {!otherMode ? (
        <div className="flex flex-col gap-2.5">
          <SearchSelect
            label="Type"
            value={GEN_CATALOG_TYPES.find((t) => t.id === typeId)?.label || ""}
            placeholder="Search type (petrol, diesel…)"
            options={GEN_CATALOG_TYPES.map((t) => t.label)}
            isLight={isLight}
            onChange={(label) => {
              const hit = GEN_CATALOG_TYPES.find((t) => t.label === label);
              setTypeId(hit?.id || "");
              setBrand("");
              setModel("");
            }}
        ...
        <textarea
          ...
          rows={3}
          placeholder="e.g. Red petrol gen, 2.5kVA, no name on it"
          className={cn(
      ...
      <div className="mt-3 flex flex-col overflow-hidden rounded-[4px]">
        ...
        {otherMode ? (
          <p className={cn("px-1 text-[11px] font-medium", muted)}>
            Describe the generator, then tap the arrow.
          </p>
        ) : null}
      </div>
    </div>
