#!/usr/bin/env node
/**
 * Backfill the commercial layer for the Vulcanizer Shop catalogue:
 *   1. shop_product_trades  — every vulcanizer product mapped to its trade
 *   2. shop_seller_listings — one listing per vulcanizer product on the
 *      platform seller, carrying status + price + qty (migration 061 layer).
 * Idempotent (keyed by product_id + seller_id; update-or-insert).
 * Usage: npx tsx scripts/backfill-vulcanizer-shop.mts
 */
...
async function main() {
  ...
  for (const prod of products ?? []) {
    ...
    const { error: te } = await sb.from("shop_product_trades").upsert(
      { product_id: pid, trade_key: "vulcanizer", is_primary: true },
  ...
  console.log(`[backfill] trades=${trades} listings inserted=${listed} updated=${updated}`);
}
