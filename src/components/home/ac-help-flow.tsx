...
const URGENCY_CHIPS: {
  ...
  { id: "normal", label: AC_FINAL_COPY.normal, fee: "1x · base + call-out" },
  { id: "emergency", label: AC_FINAL_COPY.emergency, fee: "1.25x · base + call-out" },
  { id: "remote", label: AC_FINAL_COPY.remote, fee: "1.35x · base + call-out" },
  { id: "night", label: AC_FINAL_COPY.night, fee: "1.5x · base + call-out" },
];

type FinalStep = "urgency" | "photos" | "voice" | "location";

const FLOW_SESSION_KEY = "ona-ac-flow-session";
/** Guard against exceeding the ~5 MB sessionStorage quota with media data URLs. */
const FLOW_SESSION_MAX_BYTES = 3_000_000;

interface AcFlowSnapshot {
  stack: string[];
...
export function AcHelpFlow({
  ...
  const [stack, setStack] = useState<string[]>(["unit"]);
  ...
  const { urgency, setUrgency, restoreUrgency } = useAutoCalloutUrgency({
    unsafe: false,
    distanceKm: nearestProDistanceKm(visibleTechnicians, ["ac"]),
  });
  ...
  useEffect(() => {
    ...
    const snap = readSession<AcFlowSnapshot>(FLOW_SESSION_KEY);
    if (snap) {
      setStack(
        Array.isArray(snap.stack) && snap.stack.length ? snap.stack : ["unit"]
      );
  ...
  const pick = (optionId: string, label: string) => {
    ...
    push(nextAcScreen(step, optionId, nextAnswers));
  };
  ...
  const submitText = () => {
    ...
    setAnswers(nextAnswers);
    setDraft("");
    push(nextAcScreen(step, draft.trim(), nextAnswers));
  };
  ...
  const goBack = () => {
    ...
    if (acScreen(leaving)?.kind === "text") {
      setDraft(answers[leaving] || "");
    } else {
      setDraft(
        answers[prev] && acScreen(prev)?.kind === "text" ? answers[prev] : ""
      );
    }
  };
  ...
  const onPhotos = async (files: FileList | null) => {
    ...
    const room = AC_MAX_PHOTOS - photos.length;
    if (room <= 0) {
    ...
    try {
      ...
      setPhotos((prev) => [...prev, ...next].slice(0, AC_MAX_PHOTOS));
      clearAdvanceTimer();
  ...
  const send = async () => {
    ...
    const problem = [
      composeAcProblem(answers, extra, landmark),
    ]
      .filter(Boolean)
      .join("\n");
    const payload = {
            ...
            <div className="flex min-h-0 flex-1 flex-col">
              <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain scrollbar-hide">
                <p className={cn("mt-2 px-0.5 pb-2 text-[14px] font-bold capitalize leading-snug", ink)}>
                  {finalStep === "urgency"
                    ? AC_FINAL_COPY.urgency
                    : finalStep === "photos"
                      ? AC_FINAL_COPY.photos
                      : finalStep === "voice"
                        ? AC_FINAL_COPY.voice
                        : AC_FINAL_COPY.location}
                </p>
            ...
            {screen.kind === "choice" ? (
              <div className="flex flex-col gap-1">
                {(screen.options || []).map((opt, i) => {
                  const letter =
                    step === "start" ? AC_START_OPTIONS[i]?.id : undefined;
                  return (
                    <button
                      key={opt.id}
                      type="button"
