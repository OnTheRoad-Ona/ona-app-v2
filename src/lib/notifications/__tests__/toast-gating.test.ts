...
import { shouldToastNotification, type AppNotification } from "@/lib/notifications/types";
...
describe("shouldToastNotification — cancellation carve-out", () => {
  ...
  it("does NOT toast a low/normal priority cancellation", () => {
    expect(
      shouldToastNotification(base({ priority: "normal" }))
    ).toBe(false);
  });

  it("does NOT toast ordinary open_job / requests notifications", () => {
    expect(
      shouldToastNotification(base({ jobStatus: "negotiating" }))
    ).toBe(false);
    expect(
