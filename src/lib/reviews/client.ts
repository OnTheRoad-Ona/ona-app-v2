...
async function parse<T>(res: Response): Promise<ApiOk<T> | ApiErr> {
  if (!res.ok) {
    try {
      ...
      return { ok: false, message: json?.error || json?.message || res.statusText };
    } catch {
  ...
  if (!json || json?.ok === false) {
    return { ok: false, message: json?.error || json?.message || "Request failed" };
  }
...
export async function apiGetProReviews(
  proId: string
): Promise<ProReview[]> {
  const res = await fetchWithTimeout(`/api/reviews?proId=${encodeURIComponent(proId)}`);
