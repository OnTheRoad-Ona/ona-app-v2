...
const URGENCY_CHIPS: {
  ...
  { id: "normal", label: BATTERY_FINAL_COPY.normal, fee: "1x · base + call-out" },
  { id: "emergency", label: BATTERY_FINAL_COPY.emergency, fee: "1.25x · base + call-out" },
  { id: "remote", label: BATTERY_FINAL_COPY.remote, fee: "1.35x · base + call-out" },
  { id: "night", label: BATTERY_FINAL_COPY.night, fee: "1.5x · base + call-out" },
];

type FinalStep = "urgency" | "photos" | "voice" | "location" | "tow";

const FLOW_SESSION_KEY = "ona-battery-flow-session";
/** Guard against exceeding the ~5 MB sessionStorage quota with media data URLs. */
const FLOW_SESSION_MAX_BYTES = 3_000_000;

interface BatteryFlowSnapshot {
  stack: string[];
  answers: Record<string, string>;
  vehicleLabel: string;
  manualVehicles: MotoristVehicle[];
  draft: string;
  route: BatteryRoute | null;
  ...
  towChoice: "tow" | "battery" | null;
  towConfirming: boolean;
}

export function BatteryHelpFlow({
  isLight,
  onExit,
}: {
  isLight: boolean;
  ...
  const [route, setRoute] = useState<BatteryRoute | null>(null);
  const [chosenTrade, setChosenTrade] = useState<ProService>("battery");
  ...
  const [towChoice, setTowChoice] = useState<"tow" | "battery" | null>(null);
  ...
  const screen = batteryScreen(step);
  ...
  useEffect(() => {
    ...
    const snap = readSession<BatteryFlowSnapshot>(FLOW_SESSION_KEY);
    if (snap) {
      setStack(
  ...
  useEffect(() => {
    if (!hydrated) return;
    const snap: BatteryFlowSnapshot = {
      stack,
      answers,
      vehicleLabel,
      manualVehicles,
  ...
  const push = (next: string, nextAnswers: Record<string, string>) => {
    ...
    if (next === "confirm" || next === "final") {
      const resolved = resolveBatteryRoute(nextAnswers);
      setRoute(resolved);
  ...
  const pick = (optionId: string, label: string) => {
    ...
    push(nextBatteryScreen(step, optionId, nextAnswers), nextAnswers);
  };
  ...
  const submitText = () => {
    ...
    setAnswers(nextAnswers);
    setDraft("");
    push(nextBatteryScreen(step, draft.trim(), nextAnswers), nextAnswers);
  };
  ...
  const goBack = () => {
    ...
    if (batteryScreen(leaving)?.kind === "text") {
      setDraft(answers[leaving] || "");
    } else {
      setDraft(
        answers[prev] && batteryScreen(prev)?.kind === "text"
          ? answers[prev]
          : ""
      );
    }
  ...
  const onPhotos = async (files: FileList | null) => {
    ...
    const room = BATTERY_MAX_PHOTOS - photos.length;
    if (room <= 0) {
    ...
    try {
      ...
      setPhotos((prev) => [...prev, ...next].slice(0, BATTERY_MAX_PHOTOS));
      clearAdvanceTimer();
  ...
  const send = async () => {
    ...
    const problem = [
      towChoice === "tow"
        ...
        : "",
      composeBatteryProblem(answers, extra, landmark),
    ]
      .filter(Boolean)
      .join("\n");
    const payload = {
            ...
            <div className="flex min-h-0 flex-1 flex-col">
              <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain scrollbar-hide">
                <p className={cn("mt-2 px-0.5 pb-2 text-[14px] font-bold capitalize leading-snug", ink)}>
                  {finalStep === "urgency"
                    ? BATTERY_FINAL_COPY.urgency
                    : finalStep === "photos"
                      ? BATTERY_FINAL_COPY.photos
                      : finalStep === "voice"
                        ? BATTERY_FINAL_COPY.voice
                        : finalStep === "location"
                          ? BATTERY_FINAL_COPY.location
                  ...
                  {finalStep === "photos" ? (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        {photos.length < BATTERY_MAX_PHOTOS ? (
                          <button
                            type="button"
