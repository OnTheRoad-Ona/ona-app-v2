...
const URGENCY_CHIPS: {
  ...
  {
    id: "emergency",
    label: BODY_FINAL_COPY.emergency,
    fee: "1.25x · base + call-out",
  },
  {
    id: "remote",
    label: BODY_FINAL_COPY.remote,
...
interface BodyFlowSnapshot {
  ...
  towChoice: "tow" | "body" | null;
  towConfirming: boolean;
}

export function BodyHelpFlow({
  isLight,
  onExit,
}: {
  isLight: boolean;
  ...
  const { urgency, setUrgency, restoreUrgency } = useAutoCalloutUrgency({
    ...
    distanceKm: nearestProDistanceKm(visibleTechnicians, ["body", "towing"]),
  });
  ...
  const [towChoice, setTowChoice] = useState<"tow" | "body" | null>(null);
  ...
  useEffect(() => {
    ...
    const snap = readSession<BodyFlowSnapshot>(FLOW_SESSION_KEY);
    if (snap) {
      setStack(
  ...
  const pick = (optionId: string, label: string) => {
    ...
    push(nextBodyScreen(step, optionId, nextAnswers));
  };
  ...
  const submitText = () => {
    ...
    setAnswers(nextAnswers);
    setDraft("");
    push(nextBodyScreen(step, draft.trim(), nextAnswers));
  };
  ...
  const goBack = () => {
    ...
    if (bodyScreen(leaving)?.kind === "text") {
      setDraft(answers[leaving] || "");
    } else {
      setDraft(
        answers[prev] && bodyScreen(prev)?.kind === "text" ? answers[prev] : "",
      );
    }
  };
  ...
  const onPhotos = async (files: FileList | null) => {
    ...
    const room = BODY_MAX_PHOTOS - photos.length;
    if (room <= 0) {
    ...
    try {
      ...
      setPhotos((prev) => [...prev, ...next].slice(0, BODY_MAX_PHOTOS));
      clearAdvanceTimer();
      if (total >= BODY_MIN_PHOTOS) {
  ...
  const send = async () => {
    ...
    const problem = [
      ...
      composeBodyProblem(answers, extra, landmark),
    ]
      .filter(Boolean)
      .join("\n");
    const payload = {
          ...
          {step === "final" ? (
            <div className="flex min-h-0 flex-1 flex-col">
              <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain scrollbar-hide">
                ...
                >
                  {finalStep === "urgency"
                    ? BODY_FINAL_COPY.urgency
                    : finalStep === "photos"
                      ? BODY_FINAL_COPY.photos
                      : finalStep === "voice"
                        ? BODY_FINAL_COPY.voice
                        : finalStep === "location"
                          ? BODY_FINAL_COPY.location
                          : BODY_FINAL_COPY.tow}
                </p>
