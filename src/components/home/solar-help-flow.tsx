...
const URGENCY_CHIPS: {
  ...
  { id: "emergency", label: SOLAR_FINAL_COPY.emergency, fee: "1.25x" },
  { id: "remote", label: SOLAR_FINAL_COPY.remote, fee: "1.35x" },
  { id: "night", label: SOLAR_FINAL_COPY.night, fee: "1.5x" },
];

type FinalStep = "urgency" | "photos" | "voice" | "location" | "material";

const FLOW_SESSION_KEY = "ona-solar-flow-session";
/** Guard against exceeding the ~5 MB sessionStorage quota with media data URLs. */
const FLOW_SESSION_MAX_BYTES = 3_000_000;

interface SolarFlowSnapshot {
  stack: string[];
  ...
  route: SolarRoute | null;
  chosenTrade: ProService;
  load: string;
  supplyChoice: string | null;
}

export function SolarHelpFlow({
  isLight,
  onExit,
}: {
  isLight: boolean;
  ...
  const [route, setRoute] = useState<SolarRoute | null>(null);
  const [chosenTrade, setChosenTrade] = useState<ProService>("solar");
  ...
  const push = (next: string, nextAnswers: Record<string, string>) => {
    ...
    if (next === "confirm" || next === "final") {
      const resolved = resolveSolarRoute(nextAnswers);
      setRoute(resolved);
  ...
  const pick = (optionId: string, label: string) => {
    ...
    push(nextSolarScreen(step, optionId, nextAnswers), nextAnswers);
  };
  ...
  const submitText = () => {
    ...
    setAnswers(nextAnswers);
    setDraft("");
    push(nextSolarScreen(step, draft.trim(), nextAnswers), nextAnswers);
  };
  ...
  const goBack = () => {
    ...
    if (solarScreen(leaving)?.kind === "text") {
  ...
  const onPhotos = async (files: FileList | null) => {
    ...
    const room = SOLAR_MAX_PHOTOS - photos.length;
    if (room <= 0) {
    ...
    try {
      ...
      setPhotos((prev) => [...prev, ...next].slice(0, SOLAR_MAX_PHOTOS));
      clearAdvanceTimer();
      if (total >= SOLAR_MIN_PHOTOS) {
  ...
  const send = async () => {
    ...
    const problem = [
      ...
      composeSolarProblem(answers, extra, landmark),
    ]
      .filter(Boolean)
      .join("\n");
    const payload = {
  ...
  const photosIncomplete = photos.length < SOLAR_MIN_PHOTOS;
  const solarSendDisabled =
    finalStep === "material" &&
    (supplyChoice === null || !canAdvanceText(load));
          ...
          <div className="flex min-h-0 flex-1 flex-col">
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain scrollbar-hide">
              <div className="mt-2 flex items-center gap-1 px-0.5 pb-2">
                ...
                {finalStep === "urgency"
                  ...
                  : finalStep === "photos"
                    ? SOLAR_FINAL_COPY.photos
                    : finalStep === "voice"
                      ? SOLAR_FINAL_COPY.voice
                      : finalStep === "location"
                        ? SOLAR_FINAL_COPY.location
                ...
                {finalStep === "photos" ? (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      {photos.length < SOLAR_MAX_PHOTOS ? (
                        <button
                          type="button"
                    ...
                    >
                      {photos.length}/{SOLAR_MIN_PHOTOS} photos added — 2 are
                      required
                    </p>
                    <input
                      ref={photoRef}
            ...
            <div className="flex shrink-0 flex-col gap-2 pt-2">
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  disabled={busy || solarSendDisabled || photosIncomplete}
                  onClick={() => void send()}
