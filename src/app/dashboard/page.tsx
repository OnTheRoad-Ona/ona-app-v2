...
/**
 * Professional dashboard — 50% map / 50% panel (same demarcation style as client home).
 */
export default function TechnicianDashboardPage() {
  ...
  const roleLabel =
    registeredAs === "client"
      ? "Professional"
      : PRO_SERVICE_LABELS[registeredAs] ?? registeredAs;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-3 pb-2">
          ...
          <button
            ...
            className={cn(
              "inline-flex shrink-0 items-center gap-1.5 rounded-md border-0 px-3 py-1.5 text-xs font-bold",
              online
      ...
      {/* 50 / 50 map + panel (like client home demarcation) */}
      <div className="relative flex min-h-0 flex-1 flex-col">
        ...
        <div
          className={cn(
            "card-surface z-10 flex min-h-0 flex-col overflow-hidden rounded-t-2xl shadow-[0_-4px_20px_rgba(15,23,42,0.08)]",
            panelExpanded ? "flex-1" : "flex-[0_0_50%]"
          )}
        >
          {/* Panel handle — expand / collapse like home */}
          <div className="flex shrink-0 items-center justify-center border-b border-black/[0.06] px-3 py-2">
            <button
              type="button"
              onClick={() => setPanelExpanded((v) => !v)}
              className="inline-flex items-center gap-1 rounded-full border-0 bg-black/[0.05] px-3 py-1 text-[11px] font-semibold text-[#475569]"
              aria-label={panelExpanded ? "Show map" : "Expand panel"}
            >
              {panelExpanded ? (
                <>
          ...
          <div className="min-h-0 flex-1 space-y-2 overflow-y-auto p-3 scrollbar-hide">
            <Link
              href="/orders"
              className="card-surface flex items-center justify-between rounded-lg p-3 ring-1 ring-black/[0.04]"
            >
              <div>
                <p
                  className={cn(
                ...
                <p className="text-[11px] text-muted">
                  Accept jobs, update en-route & complete work
                </p>
              </div>
            ...
            <div className="card-surface rounded-lg p-3 ring-1 ring-black/[0.04]">
            ...
            {openJobs.length === 0 ? (
              ...
              openJobs.map((job) => (
                <article
                  key={job.id}
                  className="card-surface rounded-lg p-3 ring-1 ring-black/[0.04]"
                >
                    ...
                    {job.status === "pending" && (
                      <>
                        <Button
                          ...
                          disabled={!online}
                          onClick={() =>
                            updateRequestStatus(job.id, "accepted")
                          }
                        >
                          Accept
                        </Button>
