...
describe("ac question tree", () => {
  ...
  it("walks branches B, C, D, E, F and G to the final block", () => {
    expect(
      nextAcScreen(
        nextAcScreen(
          nextAcScreen(nextAcScreen("b_behavior", "x", {}), "x", {}),
          "x",
          {},
        ),
        "x",
    ...
    expect(
      nextAcScreen(
        nextAcScreen(nextAcScreen("c_sound", "x", {}), "x", {}),
        "x",
        {},
      ),
    ).toBe("final");
    expect(
      nextAcScreen(
        nextAcScreen(nextAcScreen("d_smell", "x", {}), "x", {}),
        "x",
        {},
      ),
    ).toBe("final");
    expect(
      nextAcScreen(
        nextAcScreen(
          nextAcScreen(nextAcScreen("e_stopped", "x", {}), "x", {}),
          "x",
          {},
        ),
        "x",
    ...
    expect(
      nextAcScreen(
        nextAcScreen(nextAcScreen("f_where", "x", {}), "x", {}),
        "x",
        {},
      ),
    ).toBe("final");
    expect(nextAcScreen(nextAcScreen("g_describe", "x", {}), "x", {})).toBe(
      "final",
    );
  });
  ...
  it("never leaves A/C even burning electrical smell or engine noise stay A/C", () => {
    ...
    expect(
      resolveAcRoute({ start: "A", a_fan: "no", a_click: "no" }).trade,
    ).toBe("ac");
    expect(
      resolveAcRoute({ start: "C", c_sound: "grinding", c_when: "also_off" })
        .trade,
    ).toBe("ac");
    expect(resolveAcRoute({ start: "G", g_related: "engine" }).trade).toBe(
      "ac",
    );
    expect(resolveAcRoute({ start: "G", g_related: "electrical" }).trade).toBe(
      "ac",
    );
  ...
  it("excludes Tow from the branch G sub-categories", () => {
    expect(
      AC_SCREENS.g_related.options?.some((o) =>
        /tow|towed|cannot move/i.test(o.label),
      ),
    ).toBe(false);
  });
